...
public class Speedometer {
    ...
    // TIME_MAGIC_NUMBER calculated using a separate client-side speedometer mod's speed reading, and the distance given and solving for time using the formula speed = distance / time
    ...
    public static void displaySpeedometer(PlayerMoveEvent playerMoveEvent, HorseHighway plugin) {
        if (plugin.getHorseHighwayConfig().showSpeedometer()) { //
            ...
            playerMoveEvent.getPlayer().spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(message));
